...
<head>
    ...
    <script> 
						...
						if (serverData.success == 'true') {
							...
							if (serverData.msg.length > 0) {
								for (var i = 0; i < serverData.msg.length; i++) {
									...
									html += "<a href="+"../servlet/otherweb?requestpage=liquid/DoViewCallLiquidRequest&backpage=CallLiquidSearch&openid="+'${openid}'+"&callliquidid="+serverData.msg[i].ID+"&stationids="+'${stationids}'+"&status="+'${status}'+"&beginDate="+'${beginDate}'+"&endDate="+'${endDate}'+">"
		...
		function searchCallLiquidRequest(){
			 ...
			 var carNo=$('#carNo').val();
			$(location).attr('href', '../servlet/otherweb?requestpage=CallLiquidSearch&openid=${openid}&stationids='+stationids+'&status='+status+'&beginDate='+beginDate+'&endDate='+endDate+'&carNo='+encodeURI(encodeURI(carNo)));
		};
	</script>
</head>
<body>
	<div class="header">
		...
		<div class="center position"><b>叫液单查询</b></div>
		...
		<c:forEach var="callliquidRequest"  items="${callliquidRequest}">
			<div class="weui-form-preview">
	            <div class="weui-form-preview__hd">
	                <a href="../servlet/otherweb?requestpage=liquid/DoViewCallLiquidRequest&backpage=CallLiquidSearch&openid=${openid}&callliquidid=${callliquidRequest['ID']}&stationids=${stationids}&status=${status}&beginDate=${beginDate}&endDate=${endDate}" >
